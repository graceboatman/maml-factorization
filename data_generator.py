...
class DataGenerator(object):
    ...
    def __init__(self, num_samples_per_class, batch_size, config={}):
        ...
        if FLAGS.datasource == 'sinusoid':
            ...
            self.dim_input = np.prod(self.img_size)*3
            self.dim_output = self.num_classes
            folders = config.get('folders', './data/cifar100/train')
            # if FLAGS.test_set:
            #     metaval_folder = config.get('metaval_folder', './data/cifar100/test')
            # else:
            #     metaval_folder = config.get('metaval_folder', './data/cifar100/val')
            #
            random.seed(1)
            folders = [os.path.join(folders, label) \
                for label in os.listdir(folders) \
                if os.path.isdir(os.path.join(folders, label)) \
                ]
            random.shuffle(folders)
            self.metatrain_character_folders = folders[:65]
            if FLAGS.test_set:
                self.metaval_character_folders = folders[80:]
            else:
                self.metaval_character_folders = folders[65:80]
            # metaval_folders = [os.path.join(metaval_folder, label) \
            #     for label in os.listdir(metaval_folder) \
            #     if os.path.isdir(os.path.join(metaval_folder, label)) \
            #     ]
    ...
    def make_autoencoder_data_tensor(self, train=True):
        if train:
            folders = self.metatrain_character_folders
            # number of tasks, not number of meta-iterations. (divide by metabatch size to measure)
        ...
        # make list of files
        print('Generating autoencoder filenames')
        all_filenames = []
        ### all_filenames is just a really long list (2,000,000 len) of filename paths, where every 2 are from the same character and every 10 are from 5 different random characters regardless of Alphabet
        for _ in range(num_total_batches):
            ##### GET num_classes (N) images
            sampled_character_folders = random.sample(folders, 1)
            ##### SHUFFLE These images
            #random.shuffle(sampled_character_folders)
            ###### Get the images ---> see utils script
            labels_and_images = get_images(sampled_character_folders, [0], shuffle=False)
            # make sure the above isn't randomized order
            #labels = [li[0] for li in labels_and_images]
            filenames = [li[1] for li in labels_and_images]
        ...
        num_preprocess_threads = 1 # TODO - enable this to be set to >1
        min_queue_examples = 256
        examples_per_batch = 1 #### 5 * 2 = 10
        batch_image_size = 32  ### FLAGS.meta_batch_size * 10
        print('Batching images')
        images = tf.train.batch(
                [image],
                batch_size = batch_image_size,
        ...
        for i in range(32):
            ...
            # new_list =  []
            # for k in range(1):
            #     new_list.append(tf.gather(image_batch))
            #     if FLAGS.datasource == 'omniglot': # and FLAGS.train:
            #         new_list[-1] = tf.stack([tf.reshape(tf.image.rot90(
            #             tf.reshape(new_list[-1][ind], [self.img_size[0],self.img_size[1],1]),
            #             k=tf.cast(rotations[0,class_idxs[ind]], tf.int32)), (self.dim_input,))
        ...
        return all_image_batches


    def make_autoencoder_test_data_tensor(self, train=True):
        if train:
            folders = self.metatrain_character_folders
            # number of tasks, not number of meta-iterations. (divide by metabatch size to measure)
            num_total_batches = len(folders)
        else:
            folders = self.metaval_character_folders
            num_total_batches = len(folders)
