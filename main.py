...
flags.DEFINE_integer('autoencoder_iterations', 2000, 'number of autoencoder iterations.') # 15k for omniglot, 50k for sinusoid
...
def train(model, saver, sess, exp_string, data_generator, resume_itr=0):
    ...
    #         feed_dict = {autoencoder.X: X}
    #
    #     input_tensors = [autoencoder.train_op]
    #
    ...
    #     #     saver.save(sess, FLAGS.logdir + '/' + exp_string + '/autoencoder' + str(itr))
    #
    #     # sinusoid is infinite data, so no need to test on meta-validation set.
    #     if (itr!=0) and itr % TEST_PRINT_INTERVAL == 0 and FLAGS.datasource !='sinusoid':
    #         if 'generate' not in dir(data_generator):
    #             feed_dict = {}
    #             input_tensors = [autoencoder.metaval_total_loss1, autoencoder.summ_op]
    #         else:
    #             batch_x, batch_y, amp, phase = data_generator.generate(train=False)
    #             X = batch_x[:, :num_classes*FLAGS.update_batch_size, :]
    #             #inputb = batch_x[:, num_classes*FLAGS.update_batch_size:, :]
    #
    #             feed_dict = {autoencoder.X: X, autoencoder.meta_lr: 0.0}
    ...
    for itr in range(resume_itr, FLAGS.pretrain_iterations + FLAGS.metatrain_iterations):
        ...
        if (itr!=0) and itr % PRINT_INTERVAL == 0:
            if itr < FLAGS.pretrain_iterations:
                ...
                print_str = 'Iteration ' + str(itr - FLAGS.pretrain_iterations) + str(model.labela.eval())
            print_str += ': ' + str(np.mean(prelosses)) + ', ' + str(np.mean(postlosses))
            print(print_str)
            prelosses, postlosses = [], []
...
def main():
    ...
    #print('****************' + str(inputa.get_shape()))
    # random.seed(7)
    # image_tensor, _ = data_generator.make_data_tensor(train=True)
    # X = tf.reshape(image_tensor, [-1, dim_input])
    # autoencoder_input_tensors = {'X': X}
    #
    # dim_s = 100
    ...
    if FLAGS.train or not tf_data_load:
        # autoencoder.construct_model(input_tensors=autoencoder_input_tensors)
