...
## Autoencoder options
flags.DEFINE_integer('autoencoder_iterations', 3000, 'number of autoencoder iterations.') # 15k for omniglot, 50k for sinusoid
flags.DEFINE_float('autoencoder_lr', 1e-4, 'step size for autoencoder gradient update.')
    ...
    for itr in range(autoencoder_itrs):
        ...
        if (itr!=0) and itr % TEST_PRINT_INTERVAL == 0 and FLAGS.datasource !='sinusoid':
            if 'generate' not in dir(data_generator):
                ...
                feed_dict = {autoencoder.X: X, autoencoder.autoencoder_lr: 0.0}
...
def main():
    ...
    ims_for_encodings = data_generator.make_autoencoder_test_data_tensor(train=True)
    ...
    model = MAML(autoencoder, dim_input, dim_output, test_num_updates=test_num_updates)
