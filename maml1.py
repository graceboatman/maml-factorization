...
class MAML:
    ...
    def construct_model(self, input_tensors=None, prefix='metatrain_'):
        ...
        with tf.variable_scope('model', reuse=None) as training_scope:
            ...
            def task_metalearn(inp, reuse=True):
                ...
                #shape = inputa.shape[1]
                # model_s = Autoencoder(5, 100)
                # print(str(len(inputa)))
                # model_s.fit(inputa)
                #s = self.model_s.encode(inputa)
                ...
                print('alg line 6' + str(task_lossa))
    ...
    def construct_conv_L(self, dim=100):
        L = {}
    ...
    def construct_conv_s(self, dim=100):
        s = {}
        ...
        #s['conv1'] = tf.get_variable('conv1', [dim], initializer=conv_initializer, dtype=dtype)
        #s['b1'] = tf.Variable(tf.zeros([self.dim_hidden]))
        s['conv2'] = tf.Variable(tf.random_normal([dim]), name='conv2')
        #s['conv2'] = tf.get_variable('conv2', [dim], initializer=conv_initializer, dtype=dtype)
        #s['b2'] = tf.Variable(tf.zeros([self.dim_hidden]))
        s['conv3'] = tf.Variable(tf.random_normal([dim]), name='conv3')
        #s['conv3'] = tf.get_variable('conv3', [dim], initializer=conv_initializer, dtype=dtype)
        #s['b3'] = tf.Variable(tf.zeros([self.dim_hidden]))
        s['conv4'] = tf.Variable(tf.random_normal([dim]), name='conv4')
        #s['conv4'] = tf.get_variable('conv4', [dim], initializer=conv_initializer, dtype=dtype)
        #s['b4'] = tf.Variable(tf.zeros([self.dim_hidden]))

        if FLAGS.datasource == 'miniimagenet': #or FLAGS.datasource == 'cifar100':
            # assumes max pooling
            s['w5'] = tf.Variable(tf.random_normal([dim]), name='w5')
            #s['w5'] = tf.get_variable('w5', [dim], initializer=fc_initializer)
            #s['b5'] = tf.Variable(tf.zeros([self.dim_output]), name='b5')
        elif FLAGS.datasource == 'cifar100':
    ...
    def forward_conv(self, inp, weights, reuse=False, scope=''):
        ...
        print('forward_conv, hidden4 shape:')
        print(str(hidden4.get_shape()))
        if FLAGS.datasource == 'miniimagenet' or FLAGS.datasource == 'cifar100':
    ...
    def forward_conv_Ls(self, inp, L, s, reuse=False, scope=''):
        ...
        print('forward conv w1 value')
        print(str(s['conv1'].get_shape()))
        print(str(L['conv1'].get_shape()))
        print(str(w1.get_shape()))
